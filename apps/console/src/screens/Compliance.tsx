...
import type { ComplianceView, DiscountCheckView } from '@repracer/console-model';
...
/** Комплаенс Omnibus [Р-123]: проверка до объявления, отчёт по объявленным скидкам, доказательная история цен */
            ...
            <tbody>
              {view.rows.map((r) => (
                <tr key={r.announcementId} className={r.now.verdict === 'VIOLATION' ? 'dangerous' : ''}>
                  ...
                  <td><Badge tone={r.now.tone}>{r.now.verdict}</Badge> <span className="small">{r.now.text}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
