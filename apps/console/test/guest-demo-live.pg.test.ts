...
test('Р-160: гость не может НИЧЕГО изменить — отказывает база, а не интерфейс', async () => {
  ...
  /**
   * Шаг 41 [Р-170, находка 5 ревью]: переключение режима записи. Маршрут появился позже гостя, и первая его редакция была
   * открыта любому вошедшему — гость демо получал от базы отказ 500-м кодом. Проверяется именно отказ в ПРАВЕ.
   */
  const mode = await walk<{ error: { code: string } }>('shadow/mode (включить боевой режим)', 'POST', `/api/worlds/${w}/shadow/mode`,
    { token: guest, body: { channelAccountId: '00000000-0000-4000-8000-000000000001', toMode: 'LIVE', typedConfirmation: 'x' } });
  assert.equal(mode.status, 403, mode.text);
  assert.equal(mode.body.error.code, 'FORBIDDEN', `переключение режима гостем: ${mode.text.slice(0, 200)}`);
