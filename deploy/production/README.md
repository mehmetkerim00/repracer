...
## Второй регион (США) — шаг 42, задача D [Р-60]

Второй регион поднимается ТЕМ ЖЕ профилем: другой файл значений, другое имя проекта, другая база.

```
docker compose -p repracer-production-us -f deploy/production/compose.yaml \
  --env-file deploy/production/region-us.env up -d
```

Что меняется (полный список — в комментариях [region-us.env.example](region-us.env.example)):
...
| имя проекта compose | `-p repracer-production-us` |

Что НЕ меняется: образы, миграции, справочник витрин, тексты, профили планировщика, диспетчера и приёмника.

**Чего этот профиль не даёт.** Боевых записей на витринах США — у `amazon.com` и `EBAY_US` не установлена граница суток
(A-03, OQ-112), и перевод аккаунта в боевой режим отклоняет база [Р-172]. Регион США поднимается для ТЕНИ: продавец
видит решения, канал не получает ни одной записи [Р-169]. Поднимать его сейчас незачем: US-сервера у проекта нет, и это
профиль на случай, когда он появится.
