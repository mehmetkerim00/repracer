...
## 0.15 Изменения v0.15 → v0.16 (шаг 17, Р-94…Р-98)

| Сущность | Было | Стало | Решение | Где |
|---|---|---|---|---|
| Права роли пути решения | Перечень запретов (аудит, членства, тенанты, остановки человеком); всё остальное разрешено — путь решения мог создать согласие на необратимую миграцию eBay, включить Smart Pricing, менять границы, себестоимость и стратегии | Список РАЗРЕШЁННОГО: чтение конфигурации и справочников, запись намерений, решений, записей канала, наблюдений и системных остановок; всё прочее — административной роли | Р-96 | 0062, проверка 0067 |
| Блокировка товара при фиксации решения | `SELECT … FOR SHARE` на товаре правами пути решения (требует UPDATE на товар) | Функция базы `tenant_data.lock_decision_products` роли `repracer_decision_lock` | Р-96 | 0065 |
| **Новое:** `PricingHaltSample` | Автоматическое снятие писал путь решения (запись проверки + снятие) | Путь решения записывает только наблюдения выборки (без времени записи), итог и снятие вычисляет `channel_data.review_halt_by_sample` роли `repracer_halt_reviewer` | Находка 2 шага 16 | 0063, 0065 |
| Административная запись (границы, себестоимость, стратегии, товары, аккаунты, поправки, согласия) | Возможна без пользователя сессии и без следа | Только при пользователе сессии, действующем участнике тенанта строки; каждая запись — событие `admin_change.*` в `audit_event` | Р-97 | 0066, проверка 0067 |
| `User` (`platform.app_user`) | Административная роль меняла любого пользователя, включая признак второго фактора | Право изменения отозвано | Находка 7 шага 16, Р-97 | 0066 |
| `Tenant` (создание) | Существующий пользователь присоединялся к новому тенанту любой ролью, email не сверялся | Только владельцем, своим адресом и только если он уже входил; остальные — приглашением владельца | Находка 5 шага 16 | 0066 |
| **Новое:** `ExternalIdentityRevocation` | Перепривязка входа не поддерживалась (OQ-148) | Приглашение перепривязки от владельца со вторым фактором; приём отзывает прежнюю привязку, отозванный вход никого не сопоставляет; оба шага в аудите | Р-98 | 0066 |
| `PricingStrategyUndercut` | Срок хранения начинался при появлении новой версии, даже если единица записи закреплена за старой | Срок начинается, когда версия заменена И за ней не закреплена ни одна неснятая единица; закрепить единицу за версией с истекающим подрезом нельзя | OQ-151 | 0066 |
| Слепок объяснения | Проверялись имена ключей параметров | Проверяются вид и значение каждого параметра (сумма — целое, валюта — ISO, перечисление — из значений, момент — ISO 8601), формат — ровно `r80.1`, имена в `withheld` — идентификаторы | Находка 6 шага 16 | 0066 |

## 0.16 Изменения v0.16 → v0.17 (шаг 18, Р-99…Р-103)

| Сущность | Было | Стало | Решение | Где |
|---|---|---|---|---|
| `MigrationConsent` | Страж проверял роль автора и признак MFA в профиле; оператор записывал согласие от имени владельца | Только владелец, от своего имени (пользователь сессии = автор), со вторым фактором сессии | Р-101 | 0068 |
| Административная запись | Страж проверял членство; UPDATE `write_scope` и `pricing_halt` выпадал из стража (права сравнивались без столбцов) | Страж по административным столбцам и по роли участника (объявленное действие таблицы) | Р-100, находка 1 | 0068, проверка 0071 |
| `PricingHalt` (путь решения) | Вставка любых столбцов, в том числе снятия человеком | Только рабочие столбцы; перенос срока проверки — участник с правом снятия, второй фактор, аудит | Р-100, находки 4, 5 | 0068 |
| `DivergenceCase` (путь решения) | Изменение любых столбцов, в том числе разрешения человеком | Только открытие случая | Р-100, находка 4 | 0068 |
| `WriteScope.status` (путь решения) | Любые переходы | Только ACTIVE → BLOCKED; удержание и снятие блокировки — человек | Находка 4 | 0068 |
| `ExternalIdentity` | Две параллельные транзакции приёма создавали две действующие привязки | Проверка «одна действующая привязка» сериализована блокировкой; срок хранения — время жизни пользователя | Р-98, находка 3, OQ-153 | 0069 |
| **Новое:** роль `repracer_stock` | Записи остатков — только административная роль, от человека | Роль синхронизации остатка: пулы, движения, резервации; без цен и аудита | Р-102 | 0069, проверка 0071 |
| Слепок объяснения | Виды проверялись у параметров причин | Виды у каждого скалярного поля r80.1; валюта EUR/USD, идентификатор, неотрицательные счётчики, действительный момент | Находка 8 | 0070 |
