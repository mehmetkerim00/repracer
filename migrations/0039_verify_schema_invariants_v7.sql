-- 0039_verify_schema_invariants_v7.sql
-- Проверка схемы после шага 10 (последняя проверка набора): правила 0035 + диспетчер записей и ретранслятор outbox [Р-64, Р-34], курсы ЕЦБ в решении [Р-61],
-- граница суток витрины [Р-62], регион хранения — место клиента [Р-60].
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'platform.marketplace'::regclass AND attname = 'time_zone' AND NOT attnotnull)
     OR NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'platform.marketplace'::regclass AND attname = 'time_zone') THEN
    bad := bad || 'platform.marketplace.time_zone is missing or nullable (Р-62)';
  END IF;
  FOR r IN
