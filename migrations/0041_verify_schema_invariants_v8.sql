-- 0041_verify_schema_invariants_v8.sql
-- Проверка схемы после шага 11 (последняя проверка набора): правила 0039 + пояс витрины без общего значения для США [Р-65],
-- бюджет правок по подтверждённому поясу, автор и заметка ручной остановки.
...
BEGIN
  ...
  -- 27. Kill switch: ручная остановка хранит автора и заметку
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'channel_data.pricing_halt'::regclass AND conname = 'pricing_halt_manual_attributed') THEN
    bad := bad || 'manual pricing halt without a member and a note';
  END IF;
