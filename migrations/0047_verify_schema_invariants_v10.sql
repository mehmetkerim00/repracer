-- 0047_verify_schema_invariants_v10.sql
-- Проверка схемы после шага 13 (последняя проверка набора): правила 0043 + слепок по классу решения [Р-74], справочники слепка [Р-75],
-- аудит остановок [Р-76], стратегия независимо от режима [Р-77], вход и сессии (OQ-128), матрица прав (OQ-129).
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'platform.explanation_ruleset'::regclass AND tgname = 'explanation_ruleset_immutable')
     ...
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'tenant_data.price_intent_core'::regclass AND conname = 'price_intent_core_explanation_strategy')
  ...
  -- 36. OQ-128: в сессии — только хеш токена; хеш пароля, счётчик входов и сессии — через функции роли входа
  IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'platform.user_session'::regclass AND attname = 'token_sha256' AND attnotnull)
     OR EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid IN ('platform.user_session'::regclass, 'platform.app_user'::regclass)
                 AND NOT attisdropped AND (attname = 'token' OR attname LIKE '%password%'))
     OR has_table_privilege('repracer_app', 'platform.user_session', 'INSERT')
     OR (SELECT count(*) FROM pg_proc p WHERE p.pronamespace = 'security'::regnamespace AND p.prosecdef AND p.proowner = 'repracer_resolver'::regrole
          AND p.proname IN ('login_credential', 'record_failed_login', 'open_session', 'find_session', 'close_session', 'session_memberships')) <> 6 THEN
    bad := bad || 'sign-in secrets or sessions are reachable outside the sign-in functions (OQ-128)';
  END IF;
