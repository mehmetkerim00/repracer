-- 0056_verify_schema_invariants_v12.sql
-- Проверка схемы после шага 15 (последняя проверка набора): правила 0050 без сжатия в секциях (отменено Р-86) + перепроверка пола
-- с полом маржи при создании записи и при каждой отправке [Р-83], производные от данных канала вне вечного ядра [Р-85], привязка
-- identity только приглашением и второй фактор [Р-88], журнал проверок остановок и автоматическое снятие (находки 1–3),
-- день бюджета при повторе и обход только действующих единиц (C2, D2).
-- Ограничение (ретроспективное ревью шага 14, C1): правила сверяют каталог — имена, привилегии, наличие текста в функции, а не
-- поведение. Поведение — тесты БД packages/*/test/*.pg.test.ts и смоук-тесты tests/db, обязательные в CI [Р-84].
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'tenant_data.price_intent_core'::regclass AND conname = 'price_intent_core_dangerous_consistent')
  ...
  IF to_regclass('platform.user_credential') IS NOT NULL OR to_regclass('platform.user_session') IS NOT NULL
     ...
     OR NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid = to_regprocedure('security.resolve_external_identity(text, text)')
                     AND p.prosecdef AND p.proowner = 'repracer_resolver'::regrole)
     -- Р-88: привязка — только приёмом приглашения
     OR has_table_privilege('repracer_app', 'platform.external_identity', 'INSERT')
     OR to_regclass('platform.identity_invitation') IS NULL
     OR (SELECT count(*) FROM pg_proc p WHERE p.pronamespace = 'security'::regnamespace AND p.prosecdef AND p.proowner = 'repracer_resolver'::regrole
          AND p.proname IN ('accept_identity_invitation', 'invite_member', 'issue_signup_invitation')) <> 3
     OR has_function_privilege('repracer_app', 'security.issue_signup_invitation(text, bytea, interval)', 'EXECUTE') THEN
    bad := bad || 'own sign-in secrets or sessions exist, or external identities are linked other than by an accepted invitation (Р-78, Р-88)';
  END IF;
  ...
  END IF;
  -- Р-86: сжатие ядра в секциях отменено — у листовых секций нет порога, в политике хранения нет столбца порога
  IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'maintenance.retention_policy'::regclass AND attname = 'leaf_toast_tuple_target' AND NOT attisdropped)
     OR EXISTS (SELECT 1 FROM (VALUES ('channel_data.price_decision'::regclass), ('tenant_data.price_intent_core'::regclass)) AS t(tbl)
                  CROSS JOIN LATERAL pg_partition_tree(t.tbl) pt JOIN pg_class c ON c.oid = pt.relid
  ...
  -- 42. Р-83: пол — min_price и пол маржи, вычисленные заново, — сверяется при создании записи и при каждом переводе в отправку
  IF to_regprocedure('tenant_data.effective_price_floor(uuid, uuid, timestamptz)') IS NULL
     OR position('assert_price_floor' IN pg_get_functiondef('tenant_data.channel_write_before_insert()'::regprocedure)) = 0
     OR position('assert_price_floor' IN pg_get_functiondef('tenant_data.channel_write_before_update()'::regprocedure)) = 0
     OR position('effective_min_price' IN pg_get_functiondef('tenant_data.channel_write_before_update()'::regprocedure)) > 0 THEN
    bad := bad || 'a price write is not checked against the recomputed floor with the margin floor before dispatch (Р-83)';
  END IF;

  -- 43. Р-85: из вечного ядра не выводится значение канала
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'channel_data.price_decision'::regclass AND conname = 'price_decision_explanation_derives_no_channel')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'tenant_data.price_intent_core'::regclass AND conname = 'price_intent_core_explanation_derives_no_channel')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'tenant_data.price_intent_core'::regclass AND conname = 'price_intent_core_competitor_rejection_not_kept')
     OR to_regprocedure('security.channel_derived_param_keys()') IS NULL THEN
    bad := bad || 'the eternal core may keep values a competitor price is derived from (Р-85)';
  END IF;

  -- 44. Находки 1–3 и Р-88: журнал проверок остановки, автоматическое снятие, второй фактор
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'channel_data.pricing_halt_review'::regclass AND tgname = 'a_pricing_halt_review_guard')
     OR NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'channel_data.pricing_halt_review'::regclass AND tgname = 'zc_pricing_halt_review_released_in_tx'
                     AND tgdeferrable AND tginitdeferred)
     OR position('AUTO_SAMPLE' IN pg_get_functiondef('channel_data.pricing_halt_release_role_guard()'::regprocedure)) = 0
     OR position('session_mfa' IN pg_get_functiondef('tenant_data.price_stop_role_guard()'::regprocedure)) = 0
     OR position('created active' IN pg_get_functiondef('tenant_data.price_stop_role_guard()'::regprocedure)) = 0
     OR NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'tenant_data.membership'::regclass AND tgname = 'membership_role_change_guard') THEN
    bad := bad || 'halt reviews, automatic releases, released stops or second-factor actions are not guarded (findings 1–3, Р-88)';
  END IF;

  -- 45. C2, D2: день бюджета при повторе; обход — только действующие единицы
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'tenant_data.channel_write'::regclass AND tgname = 'ab_channel_write_budget_day_retry')
     OR position('tenant_data.write_scope s' IN pg_get_functiondef('maintenance.due_write_scopes(timestamptz, interval, interval, int)'::regprocedure)) = 0
     OR has_column_privilege('repracer_dispatcher', 'tenant_data.write_scope', 'scope_key', 'SELECT') THEN
    bad := bad || 'retries are charged to the creation day, or the sweep returns held scopes (C2, D2)';
  END IF;
