-- 0067_verify_schema_invariants_v14.sql
-- Проверка схемы после шага 17 — последняя в наборе. Р-93: правило проверяет ПОВЕДЕНИЕ либо не существует. Р-94: правило,
...
BEGIN
  ...
  IF has_table_privilege('repracer_admin', 'platform.app_user', 'UPDATE') THEN
    bad := bad || 'the administrative role may change users, including their second factor (step 16 finding 7, Р-97)';
  END IF;

  -- 14a. Р-97: каждая запись, которая есть только у административной роли, под стражем «действие человека» и в аудите
  FOR r IN SELECT * FROM security.admin_only_writes() LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_trigger tg
      ...
      bad := bad || format('%s: administrative %s without the person guard (Р-97)', r.table_name, r.privilege);
    END IF;
    IF r.table_name NOT IN ('tenant_data.price_stop'::regclass, 'channel_data.pricing_halt'::regclass,
                            'channel_data.pricing_halt_review'::regclass, 'tenant_data.membership'::regclass)
