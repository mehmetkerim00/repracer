-- 0071_verify_schema_invariants_v15.sql
-- Проверка схемы после шага 18 — последняя в наборе. Р-93: правило проверяет ПОВЕДЕНИЕ либо не существует. Р-94: правило,
...
BEGIN
  ...
  FOR r IN SELECT tr.table_name FROM security.table_registry tr
            WHERE tr.storage_class = 'PLATFORM' AND tr.mutation_mode = 'append_only'
              AND NOT EXISTS (SELECT 1 FROM maintenance.retention_policy rp WHERE rp.table_name = tr.table_name) LOOP
    bad := bad || format('%s: append-only platform table without a retention policy (OQ-153)', r.table_name);
  END LOOP;
