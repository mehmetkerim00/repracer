-- Шаг 32 [Р-145, Р-146]: у файла задания появляются свои стражи.
--
...
BEGIN
  IF NEW.sha256 IS DISTINCT FROM encode(sha256(convert_to(NEW.content, 'UTF8')), 'hex') THEN
    RAISE EXCEPTION 'the checksum of the file of bulk job % does not match its content: the file was not built by the one export path (Р-145)',
