...
BEGIN
  ...
  IF (SELECT ca.write_mode FROM tenant_data.channel_account ca
    ...
    /**
     * Находка 6 ревью шага 41: ветка стоит ПОСЛЕ присваивания `BLOCKED`, и тень перекрывает его — иначе теневая запись
     * осталась бы в очереди заблокированной, а «рождается завершённой» перестало бы быть правдой. Сам факт блокировки
     * единицы при этом не теряется: он уходит в параметры причины, и это единственное место, где продавец его увидит.
     */
    NEW.end_params := jsonb_build_object('channelAccountId', s.channel_account_id, 'wouldSpendBudget', NEW.budget_scope_key IS NOT NULL)
      || CASE WHEN NEW.status = 'BLOCKED' THEN jsonb_build_object('blockedUnit', true) ELSE '{}'::jsonb END;
...
/**
 * Права на столбец здесь НЕ выдаются: административная роль имеет UPDATE на всю `channel_account` с 0012 (у таблицы нет
 * столбцов пути решения — вся она административная, Р-100), и отдельный `GRANT UPDATE (write_mode)` был бы дублем,
 * который создаёт вид защиты [Р-104]. Переключение ограничивает не право на столбец, а страж «только строкой журнала».
 */
