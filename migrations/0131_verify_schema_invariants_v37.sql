-- 0131_verify_schema_invariants_v37.sql
...
-- этом шаге три настоящих дефекта: пересозданная функция панели снова стала исполняемой для PUBLIC (и значит для роли
-- остатков [Р-102]), у новой таблицы дайджеста административная роль получила запись умолчанием схемы, а закрытие
-- тенанта её не удаляло.
-- Шаг 41 [Р-169…Р-171] новых ПРАВИЛ не добавил: теневой режим держится стражами и ограничениями самой миграции 0128 и
-- проверяется своими строками каталога мутаций. Правила этой проверки применяются к нему как ко всему остальному: новая
-- таблица журнала переключений зарегистрирована, попадает в очистку тенанта и имеет срок хранения.
-- Проверка перенесена под новый номер, чтобы идти последней; история правил ниже — она переносится ЦЕЛИКОМ (находка 5
-- ревью шага 41: при переносе 0127 → 0129 семьдесят пять строк истории пропали, а правило, о котором не помнят, зачем
-- оно, снимают первым).
-- Шаг 40 [Р-165…Р-168] правил не добавил: 0126 — панель оператора платформы. У её роли нет прав НИ НА ОДНУ таблицу,
-- всё идёт функциями SECURITY DEFINER, и проверяется это смоуком и строками каталога мутаций [Р-108], а не правилом.
-- Шаг 37 [Р-160, Р-161] правил не добавил: 0124 — гостевое членство демо, язык тенанта и третий вид отметки доставки;
-- все они держатся стражами и ограничениями, у каждого своя проверка в смоуке и своя строка каталога мутаций [Р-108].
-- Шаг 36 [Р-156] правил не добавил: 0120 — таблица алертов и роль доставки, проверяются они своими смоуками
-- и строками каталога мутаций. Шаг 35 [Р-154] правил не добавил: индексы 0117 — под запросы, а 0118 — путь остатков; проверка перенесена, чтобы идти последней.
-- Изменение шага 34 [Р-143, Р-149]: правило 14е — у каждого вида задания, который принимает таблица, есть право на отмену,
-- известное матрице прав; новый вид без решения о праве отклоняется проверкой схемы, а не всплывает отказом в проде.
-- Изменение шага 32 [Р-145]: правило 14д — списки видов задания называют существующие виды; опечатка в имени молча выводила
-- бы вид из-под правила, и ни один тест этого не заметил бы, потому что каждый смотрит на свой вид.
-- Второе изменение шага 31 (задача D): стражи массового изменения считают один раз на ОПЕРАТОР, а не на строку (0110) —
-- правило «у каждой таблицы административной записи есть страж человека» смотрит на строковые триггеры и потому здесь не
-- трогается: страж человека остался строковым, а на уровень оператора ушли только подсчёты окна.
-- Шаг 31: одно новое правило [Р-143, OQ-210]. Вид задания, которому второй фактор НЕ НУЖЕН (`security.read_only_job_kinds`),
-- не может стоять в списке видов ни у одного стража массового изменения: иначе безобидное задание — выгрузка доказательства,
-- предпросмотр, экран различий — открывало бы операцию, которой второй фактор нужен. Ровно это и случилось на шаге 30 со
-- стражем широкого гардрейла, и нашла это мутационная проверка, а не правило. Теперь правило есть.
...
BEGIN
  ...
  -- 14г. Р-143 (шаг 31), OQ-210: вид задания, не требующий второго фактора, не открывает операций, которым он нужен
  FOR r IN
    -- Только обычные функции: pg_get_functiondef отказывается работать с агрегатами, а они тоже живут в схемах
    WITH guards AS MATERIALIZED (
      SELECT p.oid AS fn_oid FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname IN ('tenant_data', 'channel_data', 'security') AND p.prokind = 'f'
    )
    SELECT fn_oid::regprocedure::text AS fn, pg_get_functiondef(fn_oid) AS src FROM guards
     WHERE pg_get_functiondef(fn_oid) LIKE '%second_factor_present(ARRAY[%'
  LOOP
    FOR ro_kind IN SELECT k FROM security.read_only_job_kinds() AS k LOOP
      -- Вид ищется в аргументах вызова, а не где попало в теле: рядом стоят и списки видов самого задания
      IF r.src ~ ('second_factor_present\(ARRAY\[[^]]*''' || ro_kind || '''') THEN
        bad := bad || format('%s: a bulk job kind that needs no second factor (%s) opens an operation that needs one (Р-143)', r.fn, ro_kind);
      END IF;
    END LOOP;
  END LOOP;

  /**
   * 14д. Р-145, Р-146 (шаг 32): списки видов задания называют СУЩЕСТВУЮЩИЕ виды.
   *
   * Оба списка (`read_only_job_kinds`, `file_producing_job_kinds`) — это имена, набранные строками. Опечатка в имени не
   * ломает ничего заметного: она молча выводит вид из-под правила. «PRICE_EVIDENС» с кириллической С — и выгрузка
  ...
  /**
   * 14е. Р-143, Р-149 (шаг 34): у КАЖДОГО вида задания, который принимает таблица, есть право на отмену, и это право
   * известно матрице прав. Виды берутся пробой по копии таблицы (как в 14д), а не из текста CHECK [Р-93].
   */
  /**
   * Кандидаты берутся из текста ограничения — это единственное место, где виды перечислены, — но ПРИНИМАЕТСЯ ли вид,
   * решает проба, а не текст. Так новый вид, добавленный в CHECK, не может остаться без права незамеченным.
   */
  FOR r IN
  ...
  LOOP
    BEGIN
      ...
      IF NOT EXISTS (SELECT 1 FROM unnest(ARRAY['OWNER', 'ADMIN', 'OPERATOR', 'PRICING_MANAGER', 'INVENTORY_MANAGER', 'VIEWER']) role
        ...
        bad := bad || format('bulk job kind %s has no cancel right known to the permission matrix (Р-143)', r.kind);
      END IF;
    EXCEPTION
      WHEN check_violation THEN
        -- Вид таблицей не принимается — проверять нечего; контрольный несуществующий вид сюда и приходит
        NULL;
    END;
  END LOOP;
  ...
  FOR r IN SELECT tg.tgrelid::regclass AS t, tg.tgname, substring(pg_get_triggerdef(tg.oid) FROM 'WHEN \((.*)\) EXECUTE') AS cond
    ...
    IF NOT EXISTS (SELECT 1 FROM (VALUES
         ...
         -- Шаг 41 [Р-169]: теневая запись завершена уже при вставке, и в историю её уносит тот же обработчик, что боевую
         ('tenant_data.channel_write'::regclass, 'ea_channel_write_complete_shadow', '(new.status = ''SHADOW_HELD''::text)')
  ...
  BEGIN
    ...
    IF cap IS NULL THEN
      INSERT INTO platform.channel_capability (capability_id, version, status, valid_from, channel, region, api_mode, field, write_scope_kind,
          ...
          -- Шаг 42 [Р-172]: область записи объявляется явно — умолчания у столбца нет намеренно
    ...
    -- Шаг 41 [Р-170]: аккаунт мира проверки — БОЕВОЙ явно. Умолчание `SHADOW` держит записи в тени, и мир, собранный на
    -- умолчании, проверял бы тень вместо пола цены: его запись исчезла бы из очереди в историю ещё до утверждений
    ...
    VALUES (t, acc, 'KAUFLAND', 'verify-0061', ARRAY['de'], 'secret-ref:verify', m, 'LIVE');
