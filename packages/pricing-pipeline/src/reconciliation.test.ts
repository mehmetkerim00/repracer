...
test('Р-121: the rotation covers every product in ceil(n / size) consecutive calls, in a stable order independent of input order', () => {
  const items = ['B5', 'B1', 'B4', 'B2', 'B3'].map((ref) => ({ marketplace: 'A1PA6795UKMFR9', channelProductRef: ref, condition: 'new' }));
  const seen = new Set<string>();
  for (let cycle = 0; cycle < 3; cycle++) for (const q of rotation(items, 2, cycle)) seen.add(q.channelProductRef);
  assert.deepEqual([...seen].sort(), ['B1', 'B2', 'B3', 'B4', 'B5']);
  assert.deepEqual(rotation(items, 2, 0).map((q) => q.channelProductRef), ['B1', 'B2']);
  assert.deepEqual(rotation([...items].reverse(), 2, 1).map((q) => q.channelProductRef), ['B3', 'B4']);
  assert.deepEqual(rotation(items, 10, 7).length, 5);
  assert.deepEqual(rotation([], 2, 0), []);
  // Ревью шага 24, находка 9: 80 товаров по 20 — все за 4 вызова подряд, как бы редко их ни вызывали
