...
test('finding 1: a manual release review without the release, from a viewer, or for a released halt is refused; the real release is accepted', async () => {
  ...
  assert.match(await outcome(inTenant(pool, w.tenantId, review('membership-operator'), w.ids.dbId(standUserOf('membership-operator')))), /did not happen/,
    'a release record without the release fails at commit');
  assert.match(await outcome(inTenant(pool, w.tenantId, review('membership-viewer'), w.ids.dbId(standUserOf('membership-viewer')))), /may not release/);
  ...
  await store.releaseHalt(w.tenantId, halt.pricing_halt_id, {
    kind: 'MANUAL_RELEASE', outcome: 'RELEASED', sampleSize: 0, failedCount: 0, details: {}, membershipId: w.ids.dbId('membership-operator'),
    userId: w.ids.dbId(standUserOf('membership-operator')), note: 'Synthetic release after the check', at: now(),
  });
  assert.match(await outcome(inTenant(pool, w.tenantId, review('membership-operator'), w.ids.dbId(standUserOf('membership-operator')))), /not active/,
    'no second release record for a released halt');
});

test('finding 2: a price stop cannot be inserted already released', async () => {
  const w = await world([]);
  const insert = (released: boolean) => inTenant(pool, w.tenantId, (tx) => tx.query(
...
test('finding 3: an automatic release needs the system, a clean sample at the release and an elapsed window', async () => {
  ...
  const inUserSession = inTenant(pool, w.tenantId, async (tx) => {
