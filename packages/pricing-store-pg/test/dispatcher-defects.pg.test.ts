...
test('D1: an outcome unknown past the limit blocks the write scope with one CRITICAL alert and leaves the sweep', async () => {
  const w = await seedPricingWorld(pool, { fixtureTenantId: '10000000-0000-4000-8000-000000000150', fixtureChannelAccountId: ACCOUNT, marketplaces: ['de'], clock: now(), seed: { scopes: [scope(1)] } });
...
test('D2: a retry in a held write scope is not swept — no alert storm; the scope active again, the retry is due', async () => {
  const w = await seedPricingWorld(pool, { fixtureTenantId: '10000000-0000-4000-8000-000000000151', fixtureChannelAccountId: ACCOUNT, marketplaces: ['de'], clock: now(), seed: { scopes: [scope(2)] } });
