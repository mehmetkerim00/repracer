...
async function smoke(db) {
  ...
  const steps = [['postgres', 'tests/db/smoke_setup.sql'], ['svc_provisioning', 'tests/db/smoke_provision.sql'], ['svc_admin', 'tests/db/smoke_app.sql'],
    ...
    // Шаг 42 [Р-172…Р-174]: свойства витрин и дайджест — тем же порядком, что в prepare.sh, до закрытия тенанта A
    ['postgres', 'tests/db/smoke_us_digest.sql'],
...
const { R93_ROWS, STEP17_ROWS = [], STEP18_ROWS = [], STEP19_ROWS = [], STEP20_ROWS = [], STEP21_ROWS = [], STEP22_ROWS = [], STEP23_ROWS = [], STEP24_ROWS = [], STEP25_ROWS = [], STEP25_B_ROWS = [], STEP25_D_ROWS = [], STEP26_ROWS = [], STEP27_ROWS = [], STEP28_ROWS = [], STEP30_ROWS = [], STEP32_ROWS = [], STEP34_ROWS = [], STEP35_ROWS = [], STEP36_ROWS = [], STEP37_ROWS = [], STEP40_ROWS = [], STEP41_ROWS = [], STEP42_ROWS = [], R93_NOT_MUTATED = [] } = await import(pathToFileURL(catalogPath).href);
const rows = [...R93_ROWS, ...(process.argv.includes('--r93-only') ? [] : [...STEP17_ROWS, ...STEP18_ROWS, ...STEP19_ROWS, ...STEP20_ROWS, ...STEP21_ROWS, ...STEP22_ROWS, ...STEP23_ROWS, ...STEP24_ROWS, ...STEP25_ROWS, ...STEP25_B_ROWS, ...STEP25_D_ROWS, ...STEP26_ROWS, ...STEP27_ROWS, ...STEP28_ROWS, ...STEP30_ROWS, ...STEP32_ROWS, ...STEP34_ROWS, ...STEP35_ROWS, ...STEP36_ROWS, ...STEP37_ROWS, ...STEP40_ROWS, ...STEP41_ROWS, ...STEP42_ROWS])]
