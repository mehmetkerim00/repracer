...
export async function startScheduler(config: SchedulerConfig = loadConfig()): Promise<SchedulerProcess> {
  const sink = jsonSink();
  ...
  const scheduler = createScheduler({ state, source: jobSource(deps2), owner: config.owner, now: () => state.databaseNow(), alerts: sink.alerts });
