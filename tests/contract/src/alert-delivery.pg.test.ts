...
/**
 * Р-156 (шаг 36): алерт, который живёт только в базе, считается НЕдоставленным. Здесь четыре события поднимаются
 * НАСТОЯЩИМ путём (остановка человеком, недоверие каналу, отставание выгрузки, запись без итога дольше часа), и у
 * каждого проверяется ПИСЬМО: кому, о каком тенанте и канале, что случилось человеческим языком и какое первое действие.
 * Письма перехватывает модель провайдера, как модель ClickHouse и модель очереди SQS. Данные синтетические.
 */
