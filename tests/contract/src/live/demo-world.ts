...
/**
 * Старт виртуального мира: БЛИЖАЙШИЕ 09:00 UTC строго после текущего момента.
 *
 * Два условия сразу, и оба — про скрытый вход «время суток»:
 * 1) старт не «сейчас», иначе прогон, начатый в 22:30, пересекает границу суток UTC, а начатый в 10:00 — нет (ревью шага 34,
 *    находка 13: три живых прогона шага 29 краснели час в сутки);
 * 2) мир уходит вперёд от часов базы не больше чем на сутки. Это важно, потому что часть проверок базы считает по `now()`:
 *    посев объявляет себестоимость за сутки ДО старта мира, а страж Р-131 спрашивает, действует ли она СЕЙЧАС. При старте
 *    «09:00 UTC завтрашних суток» себестоимость оказывалась в будущем каждый раз, когда прогон шёл до 09:00 UTC, — то есть
 *    девять часов из двадцати четырёх демо-сутки не поднимались вовсе. Нашёл это прогон шага 36 в 03:55 UTC.
 */
export function nextNineUtc(nowMs: number = Date.now()): string {
  const NINE = 9 * 3_600_000;
  return new Date((Math.floor((nowMs - NINE) / 86_400_000) + 1) * 86_400_000 + NINE).toISOString();
}
