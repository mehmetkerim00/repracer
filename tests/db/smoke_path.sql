...
-- (SQLSTATE 42501), а не другой защитой — иначе проверка не отличает удалённое право от сработавшего триггера. Всё откатывается.
...
SELECT pg_temp.expect_fail('path creates an eBay migration consent (Р-96, Р-2)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'a4000000-0000-0000-0000-000000000003', 'a2000000-0000-0000-0000-00000000000a', 'a1000000-0000-0000-0000-00000000000a', now(), 'd1', sha256('text'), 'NONE', 'I understand', now() + interval '3 days') $q$, '42501');
SELECT pg_temp.expect_fail('path records a listing migration check (Р-96, Р-2)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'a4000000-0000-0000-0000-000000000003', 'L9', now(), sha256('s'), 'READY', 'v1') $q$, '42501');
SELECT pg_temp.expect_fail('path starts a listing migration (Р-96, Р-2)', $q$
  UPDATE tenant_data.offer_mapping SET ebay_migration_status = 'MIGRATION_STARTED' WHERE offer_mapping_id = 'ad000000-0000-0000-0000-000000000001' $q$, '42501');
SELECT pg_temp.expect_fail('path opts the tenant into Kaufland Smart Pricing (Р-96, Р-12, Р-41)', $q$
  UPDATE tenant_data.tenant SET kaufland_smart_pricing_opt_in_at = now(), kaufland_smart_pricing_opt_in_by = 'a1000000-0000-0000-0000-00000000000a' $q$, '42501');
SELECT pg_temp.expect_fail('path lowers min_price (Р-96, Р-5)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'PRODUCT', 'a5000000-0000-0000-0000-000000000001', 'EUR', 'GROSS', 1, 99, 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path raises max_price (Р-96, Р-43)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'PRODUCT', 'a5000000-0000-0000-0000-000000000001', 'EUR', 'GROSS', 999999, 99, 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path changes the unit cost (Р-96, Р-83)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'a5000000-0000-0000-0000-000000000001', 99, now(), 'EUR', 1, 'MANUAL', 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path changes the minimum margin (Р-96)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'PRODUCT', 'a5000000-0000-0000-0000-000000000001', 0, 'HOLD', 99, 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path changes the fee estimate (Р-96, Р-83)', $q$
  UPDATE channel_data.fee_estimate SET fee_model = '{"feeRateBp": 0, "fixedFeeMinor": 0}' $q$, '42501');
SELECT pg_temp.expect_fail('path creates a strategy version (Р-96)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'a9000000-0000-0000-0000-000000000001', 2, 'forged', 'FIXED', '{"type":"FIXED","priceMinor":1,"deadbandMinor":0}', ARRAY['SCHEDULE'], 'ACTIVE', 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path switches a write scope to another pricing mode (Р-96)', $q$
  UPDATE tenant_data.write_scope SET pricing_mode = 'OFF' WHERE write_scope_id = 'a6000000-0000-0000-0000-000000000001' $q$, '42501');
SELECT pg_temp.expect_fail('path connects a channel account (Р-96)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', 'KAUFLAND', 'forged', 'vault://forged', 'a2000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path creates a product (Р-96)', $q$
  INSERT INTO tenant_data.product (tenant_id, sku, kind) VALUES ('a0000000-0000-0000-0000-00000000000a', 'forged', 'SIMPLE') $q$, '42501');
SELECT pg_temp.expect_fail('path moves stock (Р-96)', $q$
  INSERT INTO tenant_data.stock_movement (tenant_id, stock_pool_id, delta, reason, occurred_at) SELECT tenant_id, stock_pool_id, 1, 'ADJUSTMENT', now() FROM tenant_data.stock_pool LIMIT 1 $q$, '42501');
SELECT pg_temp.expect_fail('path schedules the review of a halt (finding 2 ревью шага 16)', $q$
  UPDATE channel_data.pricing_halt SET next_review_at = now() - interval '1 second' $q$, '42501');
SELECT pg_temp.expect_fail('path writes an automatic halt review (finding 2 ревью шага 16)', $q$
  ...
  SELECT tenant_id, pricing_halt_id, 'AUTO_SAMPLE', 'RELEASED', 1, 0 FROM channel_data.pricing_halt LIMIT 1 $q$, '42501');

-- ---------------------------------------------------------------- Р-90 (шаг 16): аудит, членства, тенанты, пользователи, вход
SELECT pg_temp.expect_fail('path reads the audit log (Р-96)', $q$ SELECT count(*) FROM audit.audit_event $q$, '42501');
SELECT pg_temp.expect_fail('path forges an audit event (finding 5, Р-90)', $q$
  ...
  VALUES ('a0000000-0000-0000-0000-00000000000a', now(), 'USER', 'a1000000-0000-0000-0000-00000000000a', 'a2000000-0000-0000-0000-00000000000a', 'pricing.stop_released', 'price_stop') $q$, '42501');
SELECT pg_temp.expect_fail('path reads memberships (Р-96)', $q$ SELECT count(*) FROM tenant_data.membership $q$, '42501');
SELECT pg_temp.expect_fail('path inserts an OWNER membership (finding 2, Р-90)', $q$
  INSERT INTO tenant_data.membership (tenant_id, user_id, role, status) VALUES ('a0000000-0000-0000-0000-00000000000a', 'b1000000-0000-0000-0000-00000000000b', 'OWNER', 'ACTIVE') $q$, '42501');
SELECT pg_temp.expect_fail('path activates a membership (finding 3, Р-90)', $q$
  UPDATE tenant_data.membership SET status = 'ACTIVE' WHERE membership_id = 'a2000000-0000-0000-0000-0000000000a0' $q$, '42501');
SELECT pg_temp.expect_fail('path creates a tenant (Р-90)', $q$ INSERT INTO tenant_data.tenant (name, data_region) VALUES ('forged', 'EU') $q$, '42501');
SELECT pg_temp.expect_fail('path creates a user (Р-90)', $q$ INSERT INTO platform.app_user (user_id, email) VALUES (gen_random_uuid(), 'forged@example.test') $q$, '42501');
SELECT pg_temp.expect_fail('path stops pricing as a person (Р-90)', $q$
  INSERT INTO tenant_data.price_stop (tenant_id, scope_type, stopped_by_membership_id, stop_note) VALUES ('a0000000-0000-0000-0000-00000000000a', 'TENANT', 'a2000000-0000-0000-0000-00000000000a', 'forged stop by the decision path') $q$, '42501');
SELECT pg_temp.expect_fail('path invites a member (Р-90)', $q$
  SELECT security.invite_member('a0000000-0000-0000-0000-00000000000a', 'x@example.test', 'VIEWER', sha256('x'), interval '1 day') $q$, '42501');
SELECT pg_temp.expect_fail('path provisions a tenant (Р-90)', $q$ SELECT security.provision_tenant(gen_random_uuid(), 'x', 'EU', '[]') $q$, '42501');
SELECT pg_temp.expect_fail('path lists the tenants of a user (finding 13, Р-90)', $q$
  SELECT * FROM security.list_user_tenants('a1000000-0000-0000-0000-00000000000a') $q$, '42501');
SELECT pg_temp.expect_fail('path resolves an external identity (finding 13, Р-90)', $q$
  SELECT * FROM security.resolve_external_identity('https://idp.example.test', 'subject') $q$, '42501');
SELECT pg_temp.expect_fail('path assumes the administrative role (Р-90)', $q$ SET ROLE repracer_admin $q$, '42501');
